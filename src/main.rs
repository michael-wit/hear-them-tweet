...
fn main() {
    ...
    thread::spawn(move || {
        let resolver_loop = req_rx.for_each(move |msg| {
            ...
            // Todo: updating connected clients
            Ok(())
        });
    ...
    thread::spawn(move || loop {
        ...
        sender_clone.unbounded_send(UpdateMessage::NewMinute);
    });
    ...
    //     rt::run(futures::lazy(move || {
    //         for key in configuration.track_keys {
    //             let future = stream_listener::create_listener(key.clone(), token.clone());
    //             rt::spawn(future);
